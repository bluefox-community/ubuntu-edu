...
export default function ModuleBlock({
  ...
  // Rail enters with x:-40 — useSafeInView forces the final state if the
  // observer callback is lost after an anchor jump (e.g. /program#module-05).
  const rail = useSafeInView<HTMLElement>({ once: true, margin: '-15% 0px' });
  const rows = useSafeInView<HTMLOListElement>({ once: true, margin: '-15% 0px' });

  // Deep-link support (/program#module-N): the SPA renders after the browser's
  // native anchor jump and Layout resets scroll to top, so re-apply the anchor
  // once mounted. Deferred to run after Layout's scrollTo(0, 0).
  useEffect(() => {
    if (window.location.hash !== `#module-${module.number}`) return;
    const t = setTimeout(() => {
      document.getElementById(`module-${module.number}`)?.scrollIntoView({ block: 'start', behavior: 'instant' });
    }, 50);
    ...
    <section id={`module-${module.number}`} className="scroll-mt-24 overflow-x-clip">
      ...
      >
        ...
        <motion.aside
          ...
          animate={rail.shown ? { opacity: 1, x: 0 } : undefined}
        ...
        <motion.ol
          ...
          initial="hidden"
          animate={rows.shown ? 'show' : undefined}
